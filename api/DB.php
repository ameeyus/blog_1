...
class DB {
    ...
    public function add_post($data, $user_id, $name, $cover, $text, $tags) {
        $this->escape_all($data, $user_id, $name, $cover, $text, $tags);
        $this->link->query(
            "INSERT INTO `posts` (`Data`, `User_id`,`Name`, `Cover`, `Content`, `Views`, `Tags`) 
                     VALUES ('$data', $user_id,'$name', '$cover', '$text', 0, '$tags')"
        );
    ...
    public function add_comment($user_id, $post_id,  $comment, $data) {
        $this->escape_all($user_id, $post_id,  $comment, $data);
        $this->link->query("INSERT INTO `comments` (`User_id`, `Post_id`, `Data`, `Text`) VALUES ($user_id, $post_id, '$data', '$comment')");
        return $this->link->errno === 0;
    }
